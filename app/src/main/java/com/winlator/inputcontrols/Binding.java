...
public enum Binding {
    ...
    /**
     * Host-side commands: handled by the app itself instead of being injected into the guest.
     *
     * <p>This replaces the old {@code keycode.id >= KEY_CUSTOM_1.id} test, which was wrong because
     * XKeycode ids are not ordered - KEY_CUSTOM_* were retrofitted into gaps, so that range also
     * swallowed the arrow keys, Del, Home/End, PgUp/PgDn, Insert, F11/F12, R-Ctrl, R-Alt and PrtScn.
     * Sixteen ordinary keys silently did nothing, including every arrow in the bundled RTS profile.
     */
    public boolean isCommand() {
        return this == KEY_VOL_UP || this == KEY_VOL_DOWN || name().startsWith("KEY_DGP_");
    }
    ...
    public String toString() {
        switch (this) {
            ...
            case KEY_KP_DEL:
                return "NUMPAD .";
            case KEY_DGP_KEYBOARD:
                return "DGP KEYBOARD";
            case KEY_DGP_MENU:
                return "DGP MENU";
            case KEY_DGP_RELATIVE_MOUSE:
                return "DGP REL MOUSE";
            case KEY_DGP_EDIT_CONTROLS:
                return "DGP EDIT CONTROLS";
            // Short on purpose, unlike the other DGP commands: ControlElement.getBindingTextAt()
            // collapses anything over 7 characters to word initials, so "DGP FAST FORWARD" would
            // reach a radial-menu sector as "DFF". These also match the "Fast"/"Slow" labels
            // DGPlayer's VPAD uses for the same actions.
            case KEY_DGP_FAST_FORWARD:
                return "FAST";
            case KEY_DGP_SLOW_MOTION:
                return "SLOW";
            case KEY_DGP_SCREENSHOT:
                return "SHOT";
            case KEY_ALT_L:
                return "L ALT";
            case KEY_ALT_R:
    ...
    public static Binding fromString(String name) {
        switch (name) {
            ...
            // versionCode 38-39 had a separate Touch/Swipe toggle. KEY_DGP_RELATIVE_MOUSE took the job
            // over (DGPlayer's MOUSE_TOUCH_SWIPE already lands on it), but layouts saved back then
            // still name the old one - without this their button would be silently dropped on load.
            case "KEY_DGP_MOUSE_MODE":
                return Binding.KEY_DGP_RELATIVE_MOUSE;
            default:
                return valueOf(name);
        }
    }
